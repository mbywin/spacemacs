...
(defun miaoboyong/comment-box (b e)
...
(defun miaoboyong/annotate-todo ()
...
(defun miaoboyong/js2-which-function ()
...
(defun miaoboyong/run-current-file ()
  ...
  (let* (
    ...
    (if (string-equal ξfSuffix "el") ; special case for emacs lisp
      ...
      (if ξprog-name
          (progn
            (message "Running…")
            (async-shell-command ξcmd-str "*miaoboyong/run-current-file output*"))
...
(defun miaoboyong/load-yasnippet ()
  (interactive)
...
(defun miaoboyong/impatient-mode-hook ()
...
;;   (interactive)
;;   (when (my-project-name-contains-substring "mbywin")
;;     (cond
