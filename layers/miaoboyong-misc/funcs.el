;;; funcs.el --- miaoboyong Layer packages File for Spacemacs
;;
;; Copyright (c) 2015-2016 miaoboyong 
;;
;; Author: miaoboyong <mbyin@gmail.com>
;;
...
(defun miaoboyong/insert-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ";")))

(defun miaoboyong/delete-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ";")
...
(defun miaoboyong/insert-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ",")))

(defun miaoboyong/delete-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ",")
...
(defun miaoboyong/tab-region (N)
  (interactive "p")
  (if (use-region-p)
      (miaoboyong/indent-region 4)               ; region was selected, call indent-region
    (insert "    ")                   ; else insert four spaces as expected
    ))

(defun miaoboyong/untab-region (N)
  (interactive "p")
  (miaoboyong/indent-region -4))

(defun miaoboyong/hack-tab-key ()
  (interactive)
  (local-set-key (kbd "<tab>") 'miaoboyong/tab-region)
  (local-set-key (kbd "<S-tab>") 'miaoboyong/untab-region)
  )

;; I'm don't like this settings too much.
;; (add-hook 'prog-mode-hook 'miaoboyong/hack-tab-key)
...
(defun miaoboyong/helm-hotspots ()
  "helm interface to my hotspots, which includes my locations,
org-files and bookmarks"
  (interactive)
  (helm :buffer "*helm: utities*"
        :sources `(,(miaoboyong//hotspots-sources))))

(defun miaoboyong//hotspots-sources ()
  ...
  `((name . "Mail and News")
    (candidates . (("Calendar" . (lambda ()  (browse-url "https://www.google.com/calendar/render")))
                   ...
                   ("Run current flie" . (lambda () (miaoboyong/run-current-file)))
...
(defun miaoboyong/now ()
  "Insert string for the current time formatted like '2:34 PM'."
  (interactive)                 ; permit invocation in minibuffer
  (insert (format-time-string "%D %-I:%M %p")))

(defun miaoboyong/today ()
...
(defun miaoboyong/open-file-with-projectile-or-counsel-git ()
  (interactive)
  (if (or (miaoboyong/vcs-project-root)
...
(defun miaoboyong/growl-timer (minutes message)
  ...
  (run-at-time (* (string-to-number minutes) 60)
               ...
               (lambda (minute message)
                 (miaoboyong/growl-notification "Emacs Reminder" message t))
               minutes
               message))

(defun miaoboyong/goto-match-paren (arg)
...
(defun miaoboyong/hidden-dos-eol ()
  "Do not show ^M in files containing mixed UNIX and DOS line endings."
  (interactive)
  (setq buffer-display-table (make-display-table))
  (aset buffer-display-table ?\^M []))

(defun miaoboyong/remove-dos-eol ()
  "Replace DOS eolns CR LF with Unix eolns CR"
  (interactive)
  (goto-char (point-min))
  (while (search-forward "\r" nil t) (replace-match "")))

(defun miaoboyong/insert-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
  (insert (miaoboyong/retrieve-chrome-current-tab-url)))

(defun miaoboyong/retrieve-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
...
(defun miaoboyong/single-lines-only ()
...
(defun miaoboyong/word-count-for-chinese ()
  "「較精確地」統計中/日/英文字數。
- 文章中的註解不算在字數內。
...
(defun miaoboyong/evil-quick-replace (beg end )
  (interactive "r")
...
(defun miaoboyong/vcs-project-root ()
...
(defun miaoboyong/iterm-shell-command (command &optional prefix)
  "cd to `default-directory' then run COMMAND in iTerm.
With PREFIX, cd to project root."
  (interactive (list (read-shell-command
                      "iTerm Shell Command: ")
                     current-prefix-arg))
  (let* ((dir (if prefix (miaoboyong/vcs-project-root)
                default-directory))
...
(defun miaoboyong/my-mc-mark-next-like-this ()
  (interactive)
  (if (region-active-p)
